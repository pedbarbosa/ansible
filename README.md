...
- `openwrt.yml` / `openwrt/` — playbooks for the OpenWrt router + APs: package management for now (upgrade + shared/router-only package sets), via the `community.openwrt` collection. Individual configuration (DNS, firewall, VLANs, wifi) will follow.
